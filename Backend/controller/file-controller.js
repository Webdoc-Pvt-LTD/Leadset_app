...
const { applyExportFilters } = require("../services/export-filters");
...
const exportTableToExcel = async (req, res) => {
  try {
    ...
    const {
      ...
      db,
      servicePool,
      removeSub: !!sub_remove,
      removeUnsub: !!unsub_remove,
      unsubDays: unsub_days,
    });
...
const sendExcelToEmail = async (req, res) => {
  try {
    ...
    const {
      ...
      db,
      servicePool,
      removeSub: uploadedFile.remove_sub == 1,
      removeUnsub: uploadedFile.remove_unsub == 1,
      unsubDays: unsub_days,
    });
