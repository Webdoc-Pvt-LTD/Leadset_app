...
import {
  UploadCloud, FileText, X, CheckCircle2, Info,
  DollarSign, Layers, Trash2, RefreshCcw, Calendar
} from 'lucide-react'

const SERVICES = ['SMS Blast', 'Email Drip', 'Cold Call', 'WhatsApp', 'Robocall']
...
export default function Upload() {
  ...
  const validate = () => {
    ...
    if (!form.schedule_time)   e.schedule_time = 'Schedule time is required.'
  ...
  const reset = () => { setForm(initialForm); setErrors({}); setSubmitted(false) }

  const Checkbox = ({ checked, onChange, label, description }) => (
    <label className="flex items-start gap-3 cursor-pointer group">
      <div className="relative mt-0.5 flex-shrink-0">
        <input type="checkbox" className="sr-only" checked={checked} onChange={onChange} />
        <div className={`w-[18px] h-[18px] rounded border-2 transition-all duration-150
          ${checked
        ...
        />
        {checked && (
          <svg className="absolute inset-0 w-[18px] h-[18px] pointer-events-none" viewBox="0 0 18 18">
            <path d="M4 9l3.5 3.5L14 6" stroke="white" strokeWidth="2.2" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        )}
      </div>
      <div>
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800">Job Queued Successfully</h2>
          <p className="text-sm text-slate-500 mt-1">
            <span className="text-indigo-600 font-semibold">"{form.job_name}"</span> has been submitted and scheduled.
          </p>
        </div>
        <div className="card text-left w-full max-w-sm text-sm space-y-3 mt-2">
          {[
            ['Service',       form.service],
            ['Scheduled',     form.schedule_time],
            ['Balance Limit', `$${form.balance_limit}`],
            ['File',          form.file?.name],
          ].map(([k, v]) => (
            <div key={k} className="flex justify-between">
              <span className="text-slate-400 font-medium">{k}</span>
              <span className="text-slate-700 font-semibold truncate ml-4 max-w-[180px]">{v}</span>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Configure and schedule a new lead generation job.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="card space-y-5">
          ...
          <div>
            ...
            />
            {errors.job_name && <p className="text-xs text-red-500 mt-1.5">{errors.job_name}</p>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              />
              {errors.schedule_time && <p className="text-xs text-red-500 mt-1.5">{errors.schedule_time}</p>}
            </div>
            ...
            <div>
              ...
              {errors.balance_limit && <p className="text-xs text-red-500 mt-1.5">{errors.balance_limit}</p>}
            </div>
          </div>
          ...
          {form.file ? (
            <div className="flex items-center gap-3 bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-slate-700 truncate">{form.file.name}</p>
                <p className="text-xs text-slate-400">{(form.file.size / 1024).toFixed(1)} KB · .txt</p>
              </div>
              <button
                type="button"
            ...
            >
              <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors
                ...
                <UploadCloud size={22} className={dragOver ? 'text-indigo-600' : 'text-slate-400'} />
              </div>
              <div className="text-center">
                <p className="text-sm font-semibold text-slate-600">Drop your file here</p>
                <p className="text-xs text-slate-400 mt-0.5">
                  or <span className="text-indigo-600 font-medium">click to browse</span> · .txt only
                </p>
              </div>
              <input
                ref={fileRef}
